...
toolbox run -c $TMP_NAME sudo dnf install go helix clang-tools-extra nodejs ninja-build clang gtk3-devel cmake lldb zig -y > /dev/null 2>&1
echo [INFO] Updating npm...
toolbox run -c $TMP_NAME sudo npm install -g npm@latest > /dev/null 2>&1
echo [INFO] Installing npm packages...
toolbox run -c $TMP_NAME sudo npm install -g typescript typescript-language-server vscode-langservers-extracted > /dev/null 2>&1
